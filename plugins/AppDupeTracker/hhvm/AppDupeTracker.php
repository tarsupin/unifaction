...
/****** Find duplicate accounts ******/
	...
	(
		int $uniID		// <int> The UniID of the user we're finding duplicate accounts of.
	): array				// RETURNS <array> list of accounts and their dupe proximity.
...
/****** Run the duplication tracker ******/
	...
	(
		int $uniID			// <int> The UniID of the user we're testing.
	,	string $passHash		// <str> The password hash to dupe check.
	): void					// RETURNS <void>
...
/****** Check if there any replicated dupe hashes ******/
	...
	(
		string $type		// <str> The type of hash to check for.
	,	string $hash		// <str> The hash to match.
	,	int $uniID		// <int> The uniID that's being matched.
	): bool				// RETURNS <bool> TRUE if there was duplicate behavior, FALSE if not.
...
# weaken this algorithm for any reason. It's also algorithmically slow for a reason.
	...
	(
		string $password		// <str> The password to reference hash for dupe matching.
	): string					// RETURNS <str> the pass hash, "" on failure.
